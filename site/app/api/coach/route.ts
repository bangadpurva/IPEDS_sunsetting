...
async function askConfiguredModel(prompt: string): Promise<{ answer:string; mode:'ollama'|'huggingface'|'openai' } | null> {
  ...
  if (ollamaBase) {
    const response = await fetch(`${ollamaBase}/api/chat`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({model:process.env.OLLAMA_MODEL || 'qwen2.5:7b', stream:false, options:{num_predict:400}, messages:[{role:'user',content:prompt}]}), signal:AbortSignal.timeout(20000) });
  ...
  if (process.env.HF_TOKEN) {
    const response = await fetch('https://router.huggingface.co/v1/responses', { method:'POST', headers:{'content-type':'application/json',authorization:`Bearer ${process.env.HF_TOKEN}`}, body:JSON.stringify({model:process.env.HF_MODEL || 'openai/gpt-oss-120b:fastest',input:prompt,max_output_tokens:400}), signal:AbortSignal.timeout(20000) });
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    const response = await fetch(`${base}/responses`, { method:'POST', headers:{'content-type':'application/json',authorization:`Bearer ${process.env.OPENAI_API_KEY}`}, body:JSON.stringify({model:process.env.OPENAI_MODEL || 'gpt-5-mini',input:prompt,max_output_tokens:400}), signal:AbortSignal.timeout(20000) });
